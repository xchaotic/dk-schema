...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <xsl:for-each select="descendant-or-self::dk:group|descendant-or-self::dk:item">
            <xsl:if test="./parent::dk:metapresets"><xsl:text>
            </xsl:text></xsl:if>
        ...
        <!--dk.meta.cidref.enumeration = "<xsl:for-each select="//presets:category|//presets:group">
            <xsl:value-of select="./@xml:id"/><xsl:text>" | "</xsl:text>
        </xsl:for-each>-->
    </xsl:template>
</xsl:stylesheet>